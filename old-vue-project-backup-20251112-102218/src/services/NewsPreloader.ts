...
const PRELOADER_STATS_STORAGE_KEY = 'preloader.stats'
const PRELOADER_CONFIG_STORAGE_KEY = 'preloader.config'

export class BackgroundNewsPreloader {
  private config: IPreloaderConfig
  private stats: IPreloaderStats
  private preloaderTimer: ReturnType<typeof setInterval> | null = null
  ...
  private async loadStats(): Promise<void> {
    try {
      const savedStats = await storage.get<IPreloaderStats>(PRELOADER_STATS_STORAGE_KEY)
      if (savedStats) {
  ...
  private async loadConfig(): Promise<void> {
    try {
      const savedConfig = await storage.get<IPreloaderConfig>(PRELOADER_CONFIG_STORAGE_KEY)
      if (savedConfig) {
        this.config = { ...this.config, ...savedConfig }
      }
    } catch (error) {
      console.warn('Failed to load preloader config:', error)
    }
  }
  ...
  private async saveStats(): Promise<void> {
    try {
      await storage.set(PRELOADER_STATS_STORAGE_KEY, this.stats)
    } catch (error) {
  ...
  private async saveConfig(): Promise<void> {
    try {
      await storage.set(PRELOADER_CONFIG_STORAGE_KEY, this.config)
    } catch (error) {
      console.warn('Failed to save preloader config:', error)
    }
  }

  private async delay(ms: number): Promise<void> {
    if (typeof setTimeout !== 'function' || ms <= 0) return
    await new Promise<void>((resolve) => {
      setTimeout(resolve, ms)
    })
  }

  private createTimeoutSignal(timeout: number): { signal: AbortSignal; cancel?: () => void } | null {
    if (
      typeof AbortSignal !== 'undefined' &&
      typeof (AbortSignal as unknown as { timeout?: (ms: number) => AbortSignal }).timeout === 'function'
    ) {
      const signal = (AbortSignal as unknown as { timeout: (ms: number) => AbortSignal }).timeout(timeout)
      return { signal }
    }
  ...
  private async fetchWithRetry(source: IFeedSource, attempt = 1): Promise<void> {
    ...
    const timeoutController = this.createTimeoutSignal(30000)
  ...
  private startPreloader(): void {
    ...
    if (typeof setInterval !== 'function') {
      console.warn('Interval scheduling is unavailable. News preloader disabled.')
      return
    }
  ...
  private clearTimer(): void {
    ...
    this.stats.nextPreloadTime = null
  }
