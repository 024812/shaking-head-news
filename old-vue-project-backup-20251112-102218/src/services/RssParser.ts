...
export class AdvancedRssParser {
  ...
  async parseFromUrl(url: string, useProxy = true): Promise<IRssFeed> {
    ...
    try {
      ...
      if (useProxy && this.isRssUrl(url) && !url.includes('thepaper.cn')) {
        // Use CORS proxy for RSS feeds, but not for The Paper
        ...
        response = await fetch(url, {
          headers: {
            ...
            Accept: 'application/json, application/rss+xml, text/xml, application/atom+xml, text/html',
          },
          signal: AbortSignal.timeout(this.config.timeout),
        })
  ...
  async parseFeed(data: string, format: string, sourceUrl?: string): Promise<IRssFeed> {
    switch (format) {
      ...
      case 'thepaper':
        return this.parseThePaper(data, sourceUrl)
      default:
        ...
        if (data.trim().startsWith('{')) {
          ...
          return this.parseAtom(data, sourceUrl)
        } else if (sourceUrl?.includes('thepaper.cn')) {
          return this.parseThePaper(data, sourceUrl)
        } else {
  ...
  // Parse The Paper (澎湃新闻) website format
  private parseThePaper(data: string, sourceUrl?: string): IRssFeed {
    try {
      const parser = new DOMParser()
      const doc = parser.parseFromString(data, 'text/html')
      
      const feed: IRssFeed = {
        title: '澎湃新闻 (The Paper)',
        description: '澎湃新闻，专注时政与思想的媒体平台',
        link: sourceUrl || 'https://www.thepaper.cn',
        items: [],
      }

      // Extract news items from the page
      // The Paper's expressNews page typically contains news items in specific elements
      const newsElements = doc.querySelectorAll('.news_li, .list_li, .newsitem, [class*="news"], [class*="list"]')
      
      newsElements.forEach((element, index) => {
        if (feed.items.length >= this.config.maxItems) return
        
        // Try to extract title and link
        const titleElement = element.querySelector('a, h2, h3, h4, .title, .headline') || element
        ...
        const fullLink = link && !link.startsWith('http')
          ? (link.startsWith('/') ? `https://www.thepaper.cn${link}` : `https://www.thepaper.cn/${link}`)
          : link
        
        // Try to extract description/summary
        const descElement = element.querySelector('.desc, .summary, .content, p') || element
        const description = descElement?.textContent?.trim() || ''
        
        if (title && fullLink && this.isValidItem({ title, url: fullLink, description } as INewsItem)) {
          feed.items.push({
            id: `thepaper-${index}`,
      ...
      // If no items found with the general selector, try a more specific approach
      if (feed.items.length === 0) {
        ...
        allLinks.forEach((link, index) => {
          ...
          if (href && title && this.isNewsLink(href) && title.length > 10) {
            const fullLink = href.startsWith('http') ? href :
              (href.startsWith('/') ? `https://www.thepaper.cn${href}` : `https://www.thepaper.cn/${href}`)
            
            feed.items.push({
              id: `thepaper-link-${index}`,
              title: this.stripHtmlTags(title),
              description: '',
              url: fullLink,
              publishedAt: new Date().toISOString(),
      ...
      console.error('The Paper parsing error:', error)
      throw error
    }
  }

  // Helper method to check if a link might be a news article
  private isNewsLink(url: string): boolean {
    return url.includes('news') ||
           url.includes('detail') ||
           url.includes('content') ||
           url.includes('article') ||
           url.includes('/con') ||
           url.length > 20 // Avoid very short URLs that are likely navigation
  }
  ...
  private detectFormat(contentType: string, url: string): string {
    ...
    if (url.includes('thepaper.cn')) return 'thepaper'
