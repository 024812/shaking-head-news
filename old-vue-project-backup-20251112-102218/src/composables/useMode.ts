...
export const useMode = () => {
  ...
  const update = () => {
    ...
    if (mode.value === Mode.Continuous && shouldRotate.value) {
      const angle = Math.abs(value.turn) * 360
      analytics.trackRotation(mode.value, angle, value.interval * 1000)
    }
  ...
  const togglePause = () => {
    ...
    analytics.trackEvent('toggle_pause', 'mode', isPaused.value ? 'paused' : 'resumed')
  }
  ...
  watch(mode, (newMode, oldMode) => {
    if (oldMode && newMode !== oldMode) {
      analytics.trackModeChange(oldMode, newMode)
    }
  })

  watch(continuousModeInterval, (newInterval, oldInterval) => {
    if (oldInterval !== undefined && newInterval !== oldInterval) {
      analytics.trackEvent('interval_change', 'mode', 'continuous_interval', newInterval)
    }
  })

  watch(shouldRotate, (enabled, previous) => {
    if (previous !== undefined && enabled !== previous) {
      analytics.trackEvent('rotation_toggle', 'rotation', enabled ? 'enabled' : 'disabled')
    }
  })
