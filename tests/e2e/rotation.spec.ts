...
test.describe('Page Rotation Functionality', () => {
  ...
  test('should have pause/play button', async ({ page }) => {
    ...
    const pauseButton = page.locator('button[aria-label*="暂停"], button[aria-label*="Pause"], button:has-text("暂停"), button:has-text("Pause")')
    const playButton = page.locator('button[aria-label*="播放"], button[aria-label*="Play"], button:has-text("播放"), button:has-text("Play")')
